...
let map=L.map('map',{zoomControl:false}).setView([13.5795,44.0210],13);
let osm=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OSM'}).addTo(map);
let esri=L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{maxZoom:18});
let current=osm;
function setMap(t){
 if(current) map.removeLayer(current);
 if(t==='sat'){current=esri;current.addTo(map);}else{current=osm;current.addTo(map);}
 setTimeout(()=>map.invalidateSize(),200);
}
setTimeout(()=>{map.invalidateSize(); map.setView([13.5795,44.0210],13);},500);
